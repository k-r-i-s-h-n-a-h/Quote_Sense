...
export default function ProjectHub({ project = DUMMY_PROJECT }: ProjectHubProps) {
  ...
  const selectVendorQuotes = useCallback(
    (vendorId: string) => {
      const vendor = project.vendors.find((v) => v.id === vendorId);
      if (!vendor) return;
      setSelectedQuoteIds(new Set(vendor.quotes.map((q) => q.id)));
    },
    [project.vendors]
  );

  const clearSelection = useCallback(() => setSelectedQuoteIds(new Set()), []);

  const navigateToCompare = useCallback(
    (quoteIds: string[]) => {
      if (quoteIds.length < 2) return;
      const params = new URLSearchParams({
        quotes: quoteIds.join(","),
    ...
    <div className="min-h-screen bg-[#f8fafc] pb-28">
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-6 pt-6">
        <div className="rounded-xl border border-dashed border-slate-200 bg-white/60 px-4 py-3 text-xs text-slate-500 leading-relaxed">
          <strong className="text-slate-700 font-medium">Compare quotes</strong> — select 2 or more
          proposals from the same vendor or across different vendors, then click{" "}
          <strong className="text-slate-700">Compare quotes</strong>. API integration coming soon;
          comparison view uses dummy selection for now.
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-6 py-6 space-y-4">
        ...
        {project.vendors.map((vendor) => (
          <VendorCard
            ...
            onCompareVendorQuotes={(vendorId) => {
              selectVendorQuotes(vendorId);
              const v = project.vendors.find((x) => x.id === vendorId);
              if (v && v.quotes.length >= 2) {
                navigateToCompare(v.quotes.map((q) => q.id));
              }
            }}
          />
        ))}
